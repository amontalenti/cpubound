...
defmodule WordCounter do
  ...
  def run do
    ...
    word_counts =
      chunks
      |> Enum.map(fn chunk ->
        # Round-robin node selection for distribution
        node = Enum.at(available_nodes, :rand.uniform(length(available_nodes)) - 1)

        Task.Supervisor.async(supervisor, WordCounter, :count_words, [chunk], node: node)
      end)
